    ...
    public void recover(int id) {
        ...
        switch (sysLog.getType()){
            ...
            case LogType.TYPE_ROLE:
                ...
                //下面这两个方法日志更新有点特别,他不能直接调用对象方法直接就更新了，还需要调用到其他服务层
            case LogType.TYPE_ROLE_ACL:
                ...
                //上面的还原是单独的方法，在拿到值后，直接调用还原的接口方法后，单独的记录到操作日志
                //下面这两个方法,还原操作是直接调用的用户角色关系重新修改方，里面本来就已经有了日志操作记录方法了，本来的日志记录方法就是来这个log类
                //现在假如我们在这调用的用户角色关系重新修改方法，那么第二次的调用到里面的日志方法，这就不对了，那就循环了
                //所以我们直接把日志添加方法不写到这里了，写到外面去，让他在权限更新的通知一起更新到操作日志库，这里就只需要调用更新权限方法就行了
                //直接调用修改角色与用户的方法，里面还包含了日志方法了,日志也顺带更新好了
